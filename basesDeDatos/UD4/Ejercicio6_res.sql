...
select count(id) from pelicula group by genero;
...
select * from actor where not (pais_origen='EE.UU.');
-- 17. Mostrar el id de las películas que tienen más de un actor en su reparto.
select id_pelicula from reparto group by id_pelicula having count(*) > 1;
...
select * from pelicula where id not in (select id_pelicula from reparto);

-- 20. Seleccionar las películas estrenadas en los últimos 10 años.
select * from pelicula where year(now()) - 10 < anio;

-- 22 dime la moda del año de nacimiento de los actores.
select anio_nacimiento from actor group by anio_nacimiento
having count(*) = 
(select max(repeticiones) from 

(select count(*) as repeticiones, anio_nacimiento from actor 
group by anio_nacimiento) p );

-- 23 dime la media de edad de los actores agrupados por pais de nacimiento.
select avg(year(now()) - anio_nacimiento) group by pais_origen;