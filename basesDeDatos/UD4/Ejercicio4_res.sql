...
select avg(salario) as salario_medio, ciudad from Empleado
group by ciudad 
having avg(salario) > 4000;
-- 3. Encuentra el total de ventas por cada cliente.
select cliente, count(*) as importe from venta group by cliente;
...
select avg(edad),ciudad from empleado group by ciudad;
