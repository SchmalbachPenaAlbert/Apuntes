...
function quitarActivoATodos() {
    ...
    */
    ...
    TARJETAS.forEach((i) => i.classList.remove("activo"))
}
...
function prepararProductosExistentes() {
    ...
    */
    ...
    const TARJETAS = document.querySelectorAll(".producto")
    TARJETAS.forEach((tarjeta) => engancharClickProducto(tarjeta))
}
...
function anadirUnidadAlActivo() {
    ...
    */
    ...
    if (productoActivo) {
        const NUMEROPRODUCTOSACTUAL = Number(productoActivo.dataset.unidades)
        const NUEVOSPRODUCTOS = NUMEROPRODUCTOSACTUAL + 1

        productoActivo.dataset.unidades = String(NUEVOSPRODUCTOS)
        productoActivo.querySelector(".unidades").textContent = String(NUEVOSPRODUCTOS)
        resumenUnidades.textContent = String(NUEVOSPRODUCTOS)
    }
}
...
function crearLineaPedido(producto) {
    ...
    */
    ...
    const LINEAPEDIDO = document.createElement("li")
    const NOMBRE = producto.querySelector(".nombre").textContent
    const SECCION = producto.dataset.seccion
    const UNIDADES = producto.dataset.unidades
...
function pasarProductoAPedido() {
    ...
    */
    ...
    if (productoActivo) {
        const NUEVALINEA = crearLineaPedido(productoActivo)
        listaPedido.appendChild(NUEVALINEA)
    }
}
...
function crearProducto(nombre, seccion, unidades) {
    ...
    */
    ...
    PRODUCTONUEVO.tabIndex = 0

    PRODUCTONUEVO.dataset.seccion = seccion
    PRODUCTONUEVO.dataset.unidades = String(unidades)

    PRODUCTONUEVO.innerHTML = `<h3 class="nombre">${nombre}</h3>
          <p class="seccion">Sección: <span>${seccion}</span></p>
          <p class="cantidad">Pedidas: <strong class="unidades">${unidades}</strong></p>`

    engancharClickProducto(PRODUCTONUEVO)
...
function validarProducto(nombre, unidades) {
    ...
    */
    ...
    if (nombre === "" || isNaN(unidades) || unidades <= 0) {
        ok = false
    }
...
function anadirProductoDesdeFormulario() {
    ...
    */
    ...
    const SECCION = document.getElementById("nuevaSeccion").value.trim()
    const UNIDADES = Number(document.getElementById("nuevasUnidades").value)

    let datosCorrectos = true

    if (NOMBRE === "" || SECCION === "" || UNIDADES < 0) {
        datosCorrectos = false
    }

    if (datosCorrectos) {
        const TARJETA = crearProducto(NOMBRE, SECCION, UNIDADES)
        zonaProductos.appendChild(TARJETA)

        formNuevoProducto.reset()
        mostrarMensajeFormulario("Producto añadido correctamente ✅")
    } else {
        mostrarMensajeFormulario("Error: revisa nombre, sección y unidades")
    }
}

/*function borrarSeleccionado() {
  if (productoActivo) {
    const ABORRAR = productoActivo
