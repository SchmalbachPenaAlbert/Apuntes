...
select nombre as nombre_equipos, ciudad from equipos;

-- 2. Muestra los nombres de los jugadores y puntos por juego, ordenados de mayor a menor.
select nombre, puntos_por_juego from jugadores order by nombre, puntos_por_juego;

-- 3. Recupera los nombres de los equipos y la longitud de sus nombres, ordenados por esta longitud de mayor a menor.
select nombre, char_length(nombre) from equipos order by char_length(nombre);
...
select distinct ciudad, sum(campeonatos) from equipos order by sum(campeonatos);